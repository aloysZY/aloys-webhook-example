...
func serve(w http.ResponseWriter, r *http.Request, admit admitHandler) {
	sugaredLogger := logger.WithName("server")
	...
	if contentType != "application/json" {
		sugaredLogger.Errorf("contentType=%s, expect application/json", contentType)
		return
	}
	...
	case v1beta1.SchemeGroupVersion.WithKind("AdmissionReview"):
		...
		if !ok {
			sugaredLogger.Errorf("Expected v1beta1.AdmissionReview but got: %T", obj)
			return
		}
	...
	case admissionv1.SchemeGroupVersion.WithKind("AdmissionReview"):
		...
		if !ok {
			sugaredLogger.Errorf("Expected admissionv1.AdmissionReview but got: %T", obj)
			return
		}
	...
	sugaredLogger.Info(fmt.Sprintf("sending response: %v", responseObj))
	// 将响应对象序列化为JSON格式

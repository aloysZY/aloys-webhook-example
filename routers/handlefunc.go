...
import (
	...
	"github.com/aloys.zy/aloys-webhook-example/controller/back"
	"github.com/aloys.zy/aloys-webhook-example/controller/cpuOversell"
	"github.com/aloys.zy/aloys-webhook-example/controller/podDns"
...
func ServeMutateCPUOversell(writer http.ResponseWriter, request *http.Request) {
	serve(writer, request, setting.NewDelegateToV1AdmitHandler(cpuOversell.MutateCPUOversell))
}

// MutatePodDNSConfig pod创建追加DNS配置
func MutatePodDNSConfig(writer http.ResponseWriter, request *http.Request) {
	serve(writer, request, setting.NewDelegateToV1AdmitHandler(podDns.MutatePodDNSConfig))
}

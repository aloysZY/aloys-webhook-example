...
func bindFlags(cmd *cobra.Command) {
	...
	// 绑定 TLS 相关的标志
	cmd.Flags().String("tls_cert_file", "./certs/tls.crt", "File containing the default x509 Certificate for HTTPS.")
	viper.BindPFlag("service.tls_cert_file", cmd.Flags().Lookup("tls_cert_file"))

	cmd.Flags().String("tls_private_key_file", "./certs/tls.key", "File containing the default x509 private key matching --tls-cert-file.")
